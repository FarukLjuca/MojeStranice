...
<html>
<head>
	<title>Tutorijal 9, Zadatak 2</title>
	<meta charset="UTF-8">		
</head>
<body>
	<?php
		if (isset($_POST['tekst']) && isset($_POST['autor']) && isset($_POST['idVijesti'])) {
			...
			if (!$rezultat) {
	     	...
	     	echo "<h1>Hvala vam na ostavljanju komentara!</h1><a href='wt-t9z2.php'>Nazad</a>";
	     	$veza = null;
		}
		else {
	?>
	...
	<?php
		...
		if (!$rezultat) {
     	...
     	foreach ($rezultat as $vijest) {
        	...
        	echo "<h1>".$vijest['naslov']."</h1><small>".$vijest['autor']."</small><p>".$vijest['tekst']."</p><small>".date("d.m.Y. (h:i)", $vijest['vrijeme2'])."</small><br><a href='wt-t9z2-vijest.php?idVijesti=".$vijest['id']."'>".$komentarTekst."</a><br><br>";
        	echo "<form method='post' action='wt-t9z2.php'><input type='text' name='autor' placeholder='Ime autora'><br><textarea name='tekst' placeholder='Ostavite komentar'></textarea><br><input type='hidden' name='idVijesti' value='".$vijest['id']."'><input type='submit' value='Potvrdi'></form><br>";
     	}
     	$veza = null;
 	}
	?>
