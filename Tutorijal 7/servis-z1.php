<?php
    ...
    elseif($_REQUEST['akcija'] == "dodavanje") {
        $sadrzaj = file_get_contents("imenik-z1.csv");
        $objekat = json_decode($_REQUEST['objekat']);
        file_put_contents("imenik-z1.csv", $sadrzaj.$objekat->{"ime"}.",".$objekat->{"prezime"}.",".$objekat->{"brojTelefona"}."\r\n");
        print "Uspješno dodana osoba u imenik.";
    }
    elseif($_REQUEST['akcija'] == "promjena") {
        $sadrzaj = file("imenik-z1.csv");
        $noviSadrzaj = "";
        ...
        file_put_contents("imenik-z1.csv", $noviSadrzaj);
