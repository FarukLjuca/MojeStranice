function dodavanje() {
    ...
    ajax.open("post", "servis-z2.php", true);
...
function osvjezi() {
    ...
    ajax.onreadystatechange = function() {
        if(ajax.readyState == 4 && ajax.status == 200) {
            var unutrasnjostTabeleLogiran = "<tr><th>Ime</th><th>Prezime</th><th>Broj telefona</th><th>Promjena</th><th>Brisanje</th></tr>";
            var unutrasnjostTabele = "<tr><th>Ime</th><th>Prezime</th><th>Broj telefona</th></tr>";
            var objekti = JSON.parse(ajax.responseText);

            var i;
            for(i=0; i<objekti.length; i++) {
                unutrasnjostTabeleLogiran += "<tr><td style='border: 1px solid red'>" +
                ...
                objekti[i].ime + "</td><td style='border: 1px solid blue'>" +
                objekti[i].prezime + "</td><td style='border: 1px solid green'>" +
                objekti[i].brojTelefona + "</td>";
            }
            try{document.getElementById("imenik").innerHTML = unutrasnjostTabele;}
            catch(err){};
            try{document.getElementById("imenikLogiran").innerHTML = unutrasnjostTabeleLogiran;}
            catch(err){};
        }
    }
